...
const Diet = ({ selectedFoodNames, data, newDiet, colors }) => {
  ...
  return (
    <div className="flex w-3/4">
      <div className="flex flex-col basis-1/4 justify-between bg-gray-600">
        ...
        <div className="flex w-full h-20 mt-5 justify-center">
          <button
            className="bg-green-600 rounded-3xl p-3 w-44 h-14"
            onClick={newDiet}
          >
            New Diet
          </button>
      ...
      <div className="basis-3/4 overflow-y-scroll p-5">
        <h2 className="flex justify-center">Results</h2>
        <div className="flex justify-end mr-20">
          <button
            className=" bg-green-200 w-10 h-10 rounded-3xl"
            onClick={handleClick}
          >
            chart
          </button>
        ...
        {mode ? (
          <div
            className={`grid grid-cols-3 grid-rows-${
              selectedFoodNames.length + 1
            } grid-flow-row w-96 justify-self-center mt-20`}
          >
            <h2 className="flex justify-center items-center border-black border-x-2 border-t-2 h-14 font-bold">
              Food
            </h2>
            <h2 className="flex justify-center items-center border-black border-r-2 border-t-2 h-14 font-bold">
              Serving
            </h2>
            <h2 className="flex justify-center items-center border-black border-r-2 border-t-2 h-14 font-bold">
              Cost
              ...
              if (data.basicSolution[index] != 0) {
                return (
                  <>
                    <h2
                      key={name}
                      className={`flex justify-center items-center border-x-2 border-t-2 ${
                    ...
                    <h2
                      className={`flex justify-center items-center border-r-2 border-t-2 ${
                        index == selectedFoodNames.length - 1
                          ? "border-b-2"
                          : ""
                      } border-black h-14`}
                    ...
                    >
                      {(foods[name].price * data.basicSolution[index]).toFixed(
                        2
                      )}
                    </h2>
                  </>
            ...
            <h2 className="flex justify-center mt-16 w-96">
              The cost of this optimal diet is {data.Z}$ per day.
            </h2>
          </div>
        ) : (
          <DietPie
            foods={selectedFoodNames}
            serving={data.basicSolution.slice(1, selectedFoodNames.length)}
            colors={colors}
          />
        )}
      </div>
